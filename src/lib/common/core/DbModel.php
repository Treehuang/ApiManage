...
class DbModel {
    ...
    public function getAll($where = null, $value = null, $fields = null, $orderDesc = null, $limitStart = null, $listNum = null) {
        ...
        Log::info($sql, array('params'=>$condition['value']));
    ...
    protected function _parseConfig() {
        ...
        //for  easyops 名字服务 ens
        if(isset($params['name']) && isset($params['database']))
        {
            $service = ens_get_service_by_name(APP_NAME, $params['name']);
            if ($service->session_id == "") {
                Log::emergency('ens', "{$params['name']} ");
                \Flight::halt(400, 'The config data of database connect is not correct!');
                return false;
            }
            $params['dsn'] = "mysql:host={$service->ip};port={$service->port};dbname={$params['database']}";
            $this->session_id = $service->session_id;
        }
    ...
    public function __call($method, $args) {
        ...
        return \Flight::halt(500, "The method: {$method}() is not found in Model class!");
    }
    ...
    protected function setConfig() {
        $config = Configure::get( $this->_configName );
        !isset($config['name']) && $config['name'] = 'data.mysql';
        !isset($config['database']) && $config['database'] = 'anyclouds_cmdb';
