...
class DbPdo {
    ...
    public function __construct($params = array()) {
        ...
        //参数分析
        if (!$params['dsn']) {
            \Flight::halt(400, 'database config params error!');
        }
        ...
        try {
            ...
            //if (DOIT_DEBUG === false) {
                //记录错误日志
                Log::error("Database server connect error! Error Code:{$exception->getCode()} Error Message:{$exception->getMessage()}");
            ...
            //抛出异常信息
            throw new Exception('Database connect error!<br/>' . $exception->getMessage(), $exception->getCode());
        }
    ...
    public function getAll($sql, $params = array()) {
        ...
        Log::info($sql, array('params'=>$params));
        $result = $this->_execute($sql, $params);
    ...
    public function throwException($exception, $sql, $params = array()) {
        ...
        if (DOIT_DEBUG === false) {
            //记录错误日志
            Log::error("SQL execute error! SQL:{$sql} Error Message:" . $exception->getMessage());
        ...
        //抛出异常信息
        throw new Exception("SQL execute error!<br/>SQL:{$sql} " . $exception->getMessage());
    }
